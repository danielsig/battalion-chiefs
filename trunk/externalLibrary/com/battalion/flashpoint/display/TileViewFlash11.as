package com.battalion.flashpoint.display 
{
	 ...
	 */
	public final class TileViewFlash11 extends Sprite 
	{
		...
		private var _tiles : Vector.<Vector.<Image>>;
		...
		public function setTiles(tileMap : BitmapData, tileSet : Vector.<BitmapData>) : void
		{
			...
			var length : uint = _tileSet.length;
			_tileSetTextures = new Vector.<Texture>(length);
			
			for (var i : uint = 0; i < length; i++)
			{
				_tileSetTextures[i] = Texture.fromBitmapData(_tileSet[i], true, true);
			}
			
			if (!_tiles) _tiles = new Vector.<Vector.<Image>>(width);
			else _tiles.length = width;
			for (i = 0; i < width; i++)
			{
				if (!_tiles[i]) _tiles[i] = new Vector.<Image>(height);
				else _tiles[i].length = height;
				
				for (var j : uint = 0; j < height; j++)
				{
					if (!_tiles[i][j]) _tiles[i][j] = new Image(_tileSetTextures[tileIndex]);
					var tileIndex : uint = tileMap.getPixel(i, j);
					if (tileIndex >= tileSet.length) tileIndex = tileSet.length - 1;
					var bitmap : Image = _tiles[i][j];
					bitmap.texture = _tileSetTextures[tileIndex];
